    ...
    public static class ScammblerClass
    {
        public static uint[] LFSR_one(int Length, uint start, int format = 1) //Скремблер
        {
            ...
            if (format == 0)
                ...
                {
                    ShiftRegister = ((((ShiftRegister >> 9) ^ (ShiftRegister >> 3) ^ ShiftRegister) & 0x001) << 9) | (ShiftRegister >> 1);
                    output[i] = ShiftRegister | 0x01;
                }
            else
                for (int i = 0; i < Length; i++)
                {
                    ShiftRegister = ((((ShiftRegister >> 9) ^ (ShiftRegister >> 4) ^ ShiftRegister) & 0x001) << 9) | (ShiftRegister >> 1);
                    output[i] = ShiftRegister | 0x01;
                }
            return output;
        }
